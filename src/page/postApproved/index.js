...
function PostApproved () {
  ...
  const headers = ['Ảnh bìa', 'Tên bài viết', 'Người viết', 'Ngày viết', 'Ngày duyệt', 'Số lượng báo cáo']
              ...
              <TableBody>
                {posts.map((post) => (
                  <TableRow key={post._id} hover onClick={() => onRowClick(post._id)}>
                    ...
                    <TableCell>{moment(post.publishedAt).format('DD-MM-YYYY hh:mm')}</TableCell>
                    <TableCell>{post.numberOfReport}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
